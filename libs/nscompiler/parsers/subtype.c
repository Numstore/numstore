...
#include "numstore/types/subtype_builder.h"
...
parse_sub_type_inner (struct sub_type_parser *parser, error *e)
{
  ...
  struct subtype_builder builder;
  stab_create (&builder, parser->persistent);

  // VNAME
  {
    struct token *tok = parser_advance (parser->base);
    struct string vname = { .data = tok->str.data, .len = tok->str.len };
    err_t_wrap (stab_accept_vname (&builder, vname, e), e);
  }
  ...
  // Type accessors
  {
    ...
    err_t_wrap (stab_accept_type_accessor (&builder, ta, e), e);
  }

  return stab_build (parser->dest, &builder, e);
}

err_t
parse_subtype (struct parser *p, struct sub_type *dest, struct chunk_alloc *dalloc, error *e)
{
