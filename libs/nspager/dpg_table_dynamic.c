/**
 * @file dpg_table_dynamic.c
 * @brief Implementation of dynamic dirty page table
 */
...
/* Default adaptive hash table settings (same as txn_table) */
#define DEFAULT_MAX_LOAD_FACTOR 8
#define DEFAULT_MIN_LOAD_FACTOR 1
#define DEFAULT_REHASHING_WORK 28
#define DEFAULT_MAX_SIZE 2048
#define DEFAULT_MIN_SIZE 10

/**
 * @brief Equality function for hash table lookups
 *
 * Compares two entries by their page number.
 */
static bool dpg_entry_equals(const struct hnode *left, const struct hnode *right)
{
	const struct dpg_entry_dynamic *l =
	    container_of(left, const struct dpg_entry_dynamic, node);
	const struct dpg_entry_dynamic *r =
	    container_of(right, const struct dpg_entry_dynamic, node);
	return l->pg == r->pg;
}
...
err_t dpgt_dyn_open(struct dpg_table_dynamic *dest, error *e)
{
	...
	struct adptv_htable_settings settings = {
		...
		.rehashing_work = DEFAULT_REHASHING_WORK,
...
void dpgt_dyn_close(struct dpg_table_dynamic *t)
{
	/* Free all entries */
	struct hnode *node;
	while ((node = adptv_htable_lookup(&t->t, NULL,
	                                    NULL)) != NULL) {
		struct dpg_entry_dynamic *entry =
		    container_of(node, struct dpg_entry_dynamic, node);

		/* Remove from table */
		err_t err;
		error e = error_create();
		err = adptv_htable_delete(&node, &t->t, &entry->node,
		                          dpg_entry_equals, &e);
		assert(err == SUCCESS);
		(void)err;
...
err_t dpgt_dyn_add_or_update(struct dpg_table_dynamic *t, pgno pg, lsn rec_lsn, error *e)
{
	...
	if (node != NULL) {
		...
		struct dpg_entry_dynamic *entry =
		    container_of(node, struct dpg_entry_dynamic, node);
		spx_latch_lock_x(&entry->l);
		entry->rec_lsn = rec_lsn;
		spx_latch_unlock_x(&entry->l);
		spx_latch_unlock_s(&t->l);
		return SUCCESS;
	}
	spx_latch_unlock_s(&t->l);
...
static void find_min_lsn(struct hnode *node, void *ctx)
{
...
static void foreach_adapter(struct hnode *node, void *ctx)
{
...
static void merge_entry(struct dpg_entry_dynamic *entry, void *ctx)
{
...
static void serialize_entry(struct dpg_entry_dynamic *entry, void *ctx)
{
                           ...
                           error *e)
{
	...
	for (u32 i = 0; i < count; i++) {
		...
		err = dpgt_dyn_add(dest, pg, rec_lsn, e);
		if (err != SUCCESS) {
			dpgt_dyn_close(dest);
			return err;
		}
...
static void check_entry_equal(struct dpg_entry_dynamic *entry, void *ctx)
{
...
static void count_entries(struct dpg_entry_dynamic *entry, void *ctx)
{
...
static void *thread_insert_func(void *arg)
{
...
static void *thread_update_func(void *arg)
{
